---
name: "uniapp-mobile"
description: "UniApp X 移动端开发规范（HBuilderX 5.24+ 蒸汽模式），涵盖页面生命周期、组件使用、状态管理、请求拦截等。Invoke when developing uni-app x mobile applications with vapor mode."
---

# UniApp X 移动端开发规范（蒸汽模式）

> 项目级 uni-app x 开发规范，基于 **HBuilderX 5.24+** 蒸汽模式（Vapor），覆盖项目结构、生命周期、组件使用、状态管理、请求封装、编译运行等全流程。

## 蒸汽模式核心要点（HBuilderX 5.24）

| 要点            | 说明                                                                             |
| --------------- | -------------------------------------------------------------------------------- |
| **文件后缀**    | 页面 `.uvue`，工具/脚本 `.uts`，入口 `main.uts`；不支持 `.ts`/`.js` 后缀文件     |
| **Script 标签** | `<script setup>` 不写 `lang` 属性（5.25 之前不要写 `lang="ts"` 或 `lang="js"`）  |
| **语法支持**    | 蒸汽模式逻辑层由 js 引擎驱动，直接写标准 js/ts 语法，无需强类型                  |
| **CSS 方案**    | 统一用 tailwind 原子类（a-hua-unocss 编译时生成）；极个别场景才用 `<style>` 标签 |
| **原生 API**    | `UTSAndroid`/`getAndroidView` 等只能在 `utssdk/` 插件中使用，页面用 `UniElement` |
| **样式隔离**    | 仅支持 2.0 策略（scoped）                                                        |
| **Vue API**     | 仅支持组合式 API（`<script setup>`），不支持 Options API / mixin / `this`        |
| **条件编译**    | 平台标识 `#ifdef APP-ANDROID` 等仍可用                                           |
...
| **UVUE CSS**       | tailwind 原子类优先（a-hua-unocss）、UCSS 子集、不支持 gap/vh/vw、justify-content 不支持 start/end | [css-ucss](references/css-ucss.md)                                                           |
...
- [UVUE CSS](references/css-ucss.md) — 样式编写方式（tailwind 原子类优先、style 兜底）、UCSS 子集、flex 布局差异、gap 不支持、不支持 vh/vw、样式不继承
