...
describe("订单管理 - gridOptions", () => {
  ...
  it("应该有必填列", () => {
    ...
    expect(columnFields).toContain("packageName");
    expect(columnFields).toContain("studentCount");
    expect(columnFields).toContain("orderStatus");
    expect(columnFields).toContain("orderNo");
  });
  ...
  it("应该配置正确的接口地址", () => {
    expect(gridOptions.url).toBe("/sf-web/order");
  });
});
